...
const FOOTBALL_ANALYTICS_USER_ID =
  process.env.FOOTBALL_ANALYTICS_USER_ID || process.env.BWIN_USER_ID || '1zvY9nNyXMcfxdPQEyx0bIdK7r53';
const SUPABASE_URL = (process.env.SUPABASE_URL || '').trim().replace(/\/$/, '');
const SUPABASE_SERVICE_ROLE_KEY = (process.env.SUPABASE_SERVICE_ROLE_KEY || '').trim();
const GRAPH_VERSION = process.env.META_GRAPH_VERSION || 'v19.0';
const WORKER_TARGET = (process.env.FOOTBALL_ANALYTICS_NEWS_TARGET || process.env.BWIN_NEWS_TARGET || 'feed')
  .trim()
  .toLowerCase();
const IS_STORY_TARGET = WORKER_TARGET === 'stories' || WORKER_TARGET === 'story';
const WORKER_TAG = IS_STORY_TARGET ? 'football_analytics_news_story_worker' : 'football_analytics_news_worker';
const WORKER_CONFIG_BUCKET = 'worker-config';
const WORKER_CONFIG_OBJECT =
  process.env.FOOTBALL_ANALYTICS_META_CONFIG_OBJECT || process.env.BWIN_META_CONFIG_OBJECT || 'bwin-meta-accounts.json';
const NEWS_STORAGE_BUCKET = process.env.FOOTBALL_ANALYTICS_NEWS_BUCKET || process.env.BWIN_NEWS_BUCKET || 'bwin-news';
const REQUEST_USER_AGENT = 'DottMedia-FootballAnalyticsNewsWorker/1.0';
const RSS_FEEDS = [
  'https://feeds.bbci.co.uk/sport/football/rss.xml',
  'https://www.espn.com/espn/rss/soccer/news',
];
const MIN_SOURCE_IMAGE_WIDTH = Number(
  process.env.FOOTBALL_ANALYTICS_NEWS_MIN_IMAGE_WIDTH || process.env.BWIN_NEWS_MIN_IMAGE_WIDTH || 1200,
);
const MIN_SOURCE_IMAGE_HEIGHT = Number(
  process.env.FOOTBALL_ANALYTICS_NEWS_MIN_IMAGE_HEIGHT || process.env.BWIN_NEWS_MIN_IMAGE_HEIGHT || 675,
);
const MIN_SOURCE_IMAGE_BYTES = Number(
  process.env.FOOTBALL_ANALYTICS_NEWS_MIN_IMAGE_BYTES || process.env.BWIN_NEWS_MIN_IMAGE_BYTES || 80000,
);
const MAX_CANDIDATE_SCAN = Number(
  process.env.FOOTBALL_ANALYTICS_NEWS_MAX_CANDIDATES || process.env.BWIN_NEWS_MAX_CANDIDATES || 30,
);
const MAX_IMAGE_CANDIDATE_PROBES = Number(
  process.env.FOOTBALL_ANALYTICS_NEWS_MAX_IMAGE_PROBES || process.env.BWIN_NEWS_MAX_IMAGE_PROBES || 18,
);

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const repoRoot = path.resolve(__dirname, '..', '..');
const posterRendererPath = path.join(__dirname, 'render_football_analytics_news_poster.py');
...
async function getFootballAnalyticsAccounts() {
  if (SUPABASE_URL && SUPABASE_SERVICE_ROLE_KEY) {
    try {
      ...
      console.warn(
        '[football-analytics-news-worker] supabase credential config unavailable',
  ...
  initFirebase();
  const snap = await admin.firestore().collection('users').doc(FOOTBALL_ANALYTICS_USER_ID).get();
  ...
  if (!facebook.pageId || !facebook.accessToken) {
    throw new Error('Football Analytics Facebook credentials missing in Firestore');
  }
  if (!instagram.accountId || !instagram.accessToken) {
    throw new Error('Football Analytics Instagram credentials missing in Firestore');
  }
  return { facebook, instagram };
}
...
async function extractArticleStory(articleUrl, feedDescription = '') {
  ...
  try {
    ...
    console.warn(
      '[football-analytics-news-worker] article story extraction failed',
      articleUrl,
      error instanceof Error ? error.message : String(error),
    );
  }
...
async function incrementSocialDaily(postedCountByPlatform) {
  ...
  const id = `${FOOTBALL_ANALYTICS_USER_ID}_${date}`;
  ...
  if (!existing) {
    await axios.post(
      ...
      [
        {
          id,
          user_id: FOOTBALL_ANALYTICS_USER_ID,
          date,
...
async function renderFootballAnalyticsImageToTemp(sourceUrl, title) {
  const tempFile = path.join(os.tmpdir(), `football-analytics-news-${crypto.randomUUID()}.jpg`);
...
async function uploadToSupabaseStorage(fileBuffer) {
  ...
  const bucket = NEWS_STORAGE_BUCKET;
...
function buildCaptionHashtags(title, storyText = '') {
  ...
  const tags = ['#FootballNews', '#FootballUpdates', '#FootballAnalytics'];
...
async function chooseCandidate() {
  ...
  for (const feedUrl of RSS_FEEDS) {
    try {
      ...
      console.warn('[football-analytics-news-worker] feed failed', feedUrl, error instanceof Error ? error.message : String(error));
    }
  }
  ...
  for (const item of dated.slice(0, MAX_CANDIDATE_SCAN)) {
    ...
    try {
      ...
      console.warn(
        '[football-analytics-news-worker] article image extraction failed',
        item.link,
        error instanceof Error ? error.message : String(error),
      );
    }
    ...
    for (const candidateImageUrl of dedupedImageCandidates) {
      try {
        ...
        if (!isUsableImageCandidate(scored)) {
          continue;
        }
...
async function main() {
  const accounts = await getFootballAnalyticsAccounts();
  const candidate = await chooseCandidate();
  if (!candidate) {
    console.log('[football-analytics-news-worker] no fresh candidate found');
    return;
  }

  const branded = await renderFootballAnalyticsImageToTemp(candidate.imageUrl, candidate.title);
  let publicImageUrl = '';
  try {
...
main().catch(error => {
  console.error('[football-analytics-news-worker] failed', error instanceof Error ? error.stack || error.message : String(error));
  process.exit(1);
});
