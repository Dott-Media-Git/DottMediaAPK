...
import { generateReply, replyToThreadsComment } from '../services/autoReplyService';
...
type ThreadItem = { id: string; timestamp?: string; is_reply?: boolean };
type ReplyItem = {
  id: string;
  text?: string;
  timestamp?: string;
...
const fetchRecentThreads = async (target: PollTarget) => {
  ...
  return ((response.data?.data as ThreadItem[] | undefined) ?? []).filter(item => item.id && !item.is_reply);
};

const fetchReplies = async (threadId: string, accessToken: string) => {
  ...
  return (response.data?.data as ReplyItem[] | undefined) ?? [];
};
...
const reserveReply = async (reply: ReplyItem, threadId: string, target: PollTarget) => {
  const replyId = reply.id;
  const text = reply.text?.trim();
  if (!replyId || !text) return null;
  if (reply.is_reply_owned_by_me) return null;
  if (target.username && reply.username?.toLowerCase() === target.username.toLowerCase()) return null;
  if (!withinWindow(reply.timestamp)) return null;
...
const processReply = async (reply: ReplyItem, threadId: string, target: PollTarget) => {
  ...
  try {
    ...
    console.warn('[threads-comment-poll] reply failed', (error as Error).message);
  }
};
  ...
  await Promise.all(
    userIds.map(async userId => {
      try {
        ...
        const threads = (snap.data() as { socialAccounts?: { threads?: { accountId?: string; accessToken?: string; username?: string } } } | undefined)
        ...
        console.warn('[threads-comment-poll] Firestore target lookup failed', userId, (error as Error).message);
      }

      try {
        const fallback = await supabaseFallbackService.getSocialAccounts(userId);
        const threads = (fallback?.socialAccounts as { threads?: { accountId?: string; accessToken?: string; username?: string } } | undefined)
          ?.threads;
        if (threads?.accountId && threads.accessToken) {
          targets.push({
            userId,
        ...
        console.warn('[threads-comment-poll] Supabase target lookup failed', userId, (error as Error).message);
      }
    }),
  );
