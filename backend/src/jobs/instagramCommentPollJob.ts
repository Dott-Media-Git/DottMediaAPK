...
const processComment = async (comment: CommentItem, igBusinessId: string, accessToken: string) => {
  const commentId = comment.id;
  const text = comment.text?.trim();
  const fromId = comment.from?.id;
  if (!commentId || !text) return;
  if (fromId && fromId === igBusinessId) return;
  ...
  try {
    ...
    await likeInstagramComment(commentId, accessToken).catch(err =>
...
const pollOnce = async () => {
  ...
  try {
    ...
    console.warn('[ig-comment-poll] poll failed', (error as Error).message);
  }
};
