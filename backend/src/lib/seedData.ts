import { ConversationRecord, IntentCategory, Platform, PlatformStats, ResponseType } from '../types/bot';
...
const platformMix: Platform[] = ['whatsapp', 'instagram', 'facebook', 'threads', 'linkedin', 'web'];

const platformStatsTemplate = (messages: number, leads: number): PlatformStats => ({
  messages,
  leads,
...
export const sampleStats = Array.from({ length: 7 }).map((_, index) => {
  ...
  const platformBreakdown = platformMix.reduce<Record<Platform, PlatformStats>>((acc, platform, idx) => {
  ...
  return {
    ...
    newLeadsToday: leads,
    mostCommonCategory: (['Lead Inquiry', 'Demo Booking', 'Support'][index % 3] ?? 'General Chat') as IntentCategory,
...
export const sampleConversations: ConversationRecord[] = [
  {
