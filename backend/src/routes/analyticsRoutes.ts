...
const isAllowedWebTrackHost = (value?: string) => {
...
const inferTrafficSource = (input: { source?: string; utmSource?: string; referrer?: string }) => {
  const direct = normalizeTrafficSource(input.source || input.utmSource);
  if (direct !== 'web' || input.source || input.utmSource) return direct;
  ...
  try {
    ...
    if (!scopeId && !ownerId && ownerEmailRaw) {
      ...
      if (!userSnap.empty) {
        ...
        const userData = userDoc.data() as { orgId?: string };
        ownerId = userDoc.id;
...
router.get('/stats/socialLive', requireFirebase, async (req, res, next) => {
  try {
    ...
    // Rolling totals and today's window are requested in parallel. Only the
    // canonical rolling result may update liveSocialCurrent in Supabase.
    const isCanonicalCurrentSnapshot = lookbackHours === undefined || lookbackHours > 26;
    if (isCanonicalCurrentSnapshot) {
      ...
      await supabaseFallbackService.upsertMetricSummaries(
        summaryScopes
          ...
          .filter(
            ({ scopeKey }) =>
              Boolean(scopeKey) && !seenScopeKeys.has(scopeKey) && Boolean(seenScopeKeys.add(scopeKey)),
          )
          .map(({ scope, scopeKey }) => ({
            scopeKey,
            userId: scope.userId,
