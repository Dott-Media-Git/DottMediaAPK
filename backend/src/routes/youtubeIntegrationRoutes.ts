...
import {
  ...
  revealYouTubeRefreshToken,
  upsertYouTubeIntegration,
  updateYouTubeIntegrationDefaults,
...
const computeConnectUrl = (req: Request) => `${getBaseUrl(req)}/integrations/youtube/connect`;

const ensureYouTubeClientConfig = (req: Request) => {
...
router.post('/youtube/upload', requireFirebase, async (req, res, next) => {
  try {
    const authUser = (req as AuthedRequest).authUser;
    if (!authUser) throw createHttpError(401, 'Unauthorized');
