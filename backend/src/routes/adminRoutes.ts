...
const timestampToIso = (value: unknown): string | null => {
  ...
  const candidate = value as { toDate?: () => Date; toMillis?: () => number; _seconds?: number; seconds?: number };
...
router.post('/admin/orgs', requireFirebase, async (req, res, next) => {
  try {
    const user = (req as OrgRequest).authUser;
    if (!user) throw createHttpError(401, 'Auth required');
...
router.patch('/admin/orgs/:orgId', requireRole(['Owner', 'Admin']), async (req, res, next) => {
  try {
    ...
    await logAuditEvent(req.params.orgId, (req as OrgRequest).authUser!.uid, 'org.update', 'orgs', req.body);
    res.json({ org });
  } catch (error) {
    next(error);
  }
...
router.post('/admin/orgs/:orgId/users/invite', requireRole(['Owner', 'Admin']), async (req, res, next) => {
  try {
    ...
    const membership = await inviteOrgUser(req.params.orgId, uid, role, (req as OrgRequest).authUser!.uid);
...
router.patch('/admin/orgs/:orgId/users/:uid', requireRole(['Owner', 'Admin']), async (req, res, next) => {
  try {
    ...
    await logAuditEvent(req.params.orgId, (req as OrgRequest).authUser!.uid, 'user.role.update', 'orgUsers', {
      uid: req.params.uid,
      role: req.body.role,
    });
    res.json({ ok: true });
...
router.delete('/admin/orgs/:orgId/users/:uid', requireRole(['Owner', 'Admin']), async (req, res, next) => {
  try {
    ...
    await logAuditEvent(req.params.orgId, (req as OrgRequest).authUser!.uid, 'user.remove', 'orgUsers', {
      uid: req.params.uid,
    });
    res.status(204).end();
  } catch (error) {
...
router.patch('/admin/orgs/:orgId/settings', requireRole(['Owner', 'Admin']), async (req, res, next) => {
  try {
    ...
    await logAuditEvent(req.params.orgId, (req as OrgRequest).authUser!.uid, 'settings.update', 'orgSettings', req.body);
    res.json({ settings });
  } catch (error) {
    next(error);
  }
...
router.post('/admin/orgs/:orgId/channels/:channel/connect', requireRole(['Owner', 'Admin']), async (req, res, next) => {
  try {
    ...
    await logAuditEvent(req.params.orgId, (req as OrgRequest).authUser!.uid, 'channel.connect', req.params.channel, {});
...
router.delete(
  '/admin/orgs/:orgId/channels/:channel/disconnect',
  ...
  async (req, res, next) => {
    try {
      ...
      await logAuditEvent(
        req.params.orgId,
        (req as OrgRequest).authUser!.uid,
        'channel.disconnect',
        req.params.channel,
        {},
      );
...
router.post('/admin/orgs/:orgId/secret', requireRole(['Owner', 'Admin']), async (req, res, next) => {
  try {
    ...
    await logAuditEvent(req.params.orgId, (req as OrgRequest).authUser!.uid, 'secret.put', req.body.key, {});
...
router.get('/admin/orgs/:orgId/usage', requireRole(['Owner', 'Admin']), async (req, res, next) => {
  try {
    const usage = await getUsage(req.params.orgId, req.query.from as string, req.query.to as string);
    res.json({ usage });
  } catch (error) {
    next(error);
  }
...
router.post('/admin/orgs/:orgId/plan/swap', requireRole(['Owner']), async (req, res, next) => {
  try {
    const session = await swapPlan(
      req.params.orgId,
      req.body.plan,
      req.body.successUrl ?? 'https://admin.dott-media.com/success',
      req.body.cancelUrl ?? 'https://admin.dott-media.com/cancel',
    );
    await updateOrg(req.params.orgId, { plan: req.body.plan });
    await logAuditEvent(req.params.orgId, (req as OrgRequest).authUser!.uid, 'plan.swap', 'orgs', {
      plan: req.body.plan,
    });
    res.json(session);
  } catch (error) {
...
router.post('/admin/orgs/:orgId/test/webhook', requireRole(['Owner', 'Admin']), async (req, res, next) => {
  try {
    ...
    await logAuditEvent(req.params.orgId, (req as OrgRequest).authUser!.uid, 'webhook.test', 'ops', req.body);
...
router.post('/admin/orgs/:orgId/jobs/run', requireRole(['Owner', 'Admin']), async (req, res, next) => {
  try {
    const job = await enqueueJob(req.params.orgId, req.body.type, (req as OrgRequest).authUser!.uid);
    res.json(job);
  } catch (error) {
    next(error);
  }
