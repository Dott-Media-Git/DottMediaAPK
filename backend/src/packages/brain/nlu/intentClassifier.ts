 ...
 */
export async function classifyIntentText(text: string): Promise<IntentClassification> {
  if (!text?.trim()) return DEFAULT_CLASSIFICATION;
...
function normalizeIntent(intent?: string): IntentLabel {
  const upper = intent?.toUpperCase() ?? '';
  if (upper.includes('DEMO')) return 'BOOK_DEMO';
  if (upper.includes('LEAD') || upper.includes('INQUIRY')) return 'LEAD_INQUIRY';
  if (upper.includes('SUPPORT') || upper.includes('HELP')) return 'SUPPORT';
  if (upper.includes('REFERR')) return 'REFERRAL';
