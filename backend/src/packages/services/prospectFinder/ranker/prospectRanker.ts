...
import { Prospect, ProspectRankingContext } from '../types';
 ...
 */
export async function scoreProspects(prospects: Prospect[], context: ProspectRankingContext = {}): Promise<Prospect[]> {
 ...
 */
export function scoreProspect(prospect: Prospect, context: ProspectRankingContext = {}): number {
  let score = 40;
...
function shouldUseSemanticRanking(prospects: Prospect[]) {
  return process.env.ENABLE_OUTBOUND_SEMANTIC_RANKING !== 'false' && prospects.length >= 5;
}

async function semanticAdjustments(prospects: Prospect[], context: ProspectRankingContext) {
    ...
    messages: [
      {
        role: 'system',
        content:
          'You are ranking B2B prospects for AI sales outreach. Return JSON { "scores": [{ "id": "prospectId", "score": 0-100 }] }. Only include provided IDs.',
      },
      {
        role: 'user',
  ...
  try {
    const parsed = JSON.parse(content) as { scores?: Array<{ id: string; score: number }> };
