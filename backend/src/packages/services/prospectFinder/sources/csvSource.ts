...
const normalizeChannel = (value?: string) => {
  ...
  if (!raw) return 'csv';
  if (raw === 'twitter' || raw === 'x' || raw === 'x.com') return 'x';
 ...
 */
export async function loadCsvProspects(params: ProspectDiscoveryParams): Promise<ProspectSeed[]> {
  ...
  return dataRows.slice(0, params.limit ?? 100).map((row, index) => {
    ...
    return {
      ...
      channel: normalizeChannel(record.channel) as ProspectSeed['channel'],
