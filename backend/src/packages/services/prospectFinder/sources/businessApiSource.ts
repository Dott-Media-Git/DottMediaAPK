 ...
 */
export async function searchBusinessProspects(params: ProspectDiscoveryParams): Promise<ProspectSeed[]> {
  if (!params.businessQuery && !params.industry) return [];
...
async function queryBusinessDirectory(params: ProspectDiscoveryParams): Promise<ProspectSeed[]> {
  ...
  const companies = (response.data?.results?.companies as Array<Record<string, any>>) ?? [];
...
function mockBusinessProspects(params: ProspectDiscoveryParams): ProspectSeed[] {
  return [
    {
      id: 'web-1',
