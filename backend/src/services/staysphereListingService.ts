...
const normalizeListingUrl = (value: string, baseUrl: string) => {
  try {
    const url = new URL(normalizeUrl(value, baseUrl));
    url.hash = '';
    url.search = '';
    if (!/^\/properties\/[^/]+\/?$/i.test(url.pathname)) return '';
    return url.toString();
  } catch {
    return '';
  }
...
const isStayListingText = (value: string) =>
  /(furnished|short.?term|short stay|rental|rentals|apartment|apartments|villa|villas|hotel|guest house|lodge|stay)/i.test(
    value,
  ) && !/(granite|acre|acres|plot|plots|land for sale|for sale|coffee plantation|factory|warehouse)/i.test(value);
...
async function discoverAderokUrls() {
  ...
  for (const searchUrl of ADEROK_SEARCH_URLS) {
    try {
      ...
      urls.push(
        ...Array.from(html.matchAll(/https:\/\/aderokestates\.com\/properties\/[^"'\s<>#?]+\/?/gi)).map(match =>
          match[0],
        ),
      );
    } catch {
...
export function buildStaysphereCoverText(listing: StaysphereListing) {
  const area =
    listing.title.match(/\b(?:in|at)\s+([A-Za-z][A-Za-z\s-]{2,32})$/i)?.[1]?.trim() ||
  ...
  const svg = `
    <svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
      ...
      ${headlineLines
        .map(
          (line, index) =>
            `<text x="82" y="${headlineY + index * (headlineSize + 12)}" fill="#ffffff" font-family="Arial Black, Arial, Helvetica, sans-serif" font-size="${headlineSize}" font-weight="900">${escapeSvg(line)}</text>`,
        )
        .join('\n')}
