...
const fetchCardImage = async (url: string): Promise<Buffer | null> => {
  ...
  try {
    ...
    const roundedMask = Buffer.from(
      `<svg width="${IMAGE_FRAME.width}" height="${IMAGE_FRAME.height}" xmlns="http://www.w3.org/2000/svg">
    ...
    console.warn('[story-image] source image fetch failed', (error as Error).message);
    return null;
  }
};
