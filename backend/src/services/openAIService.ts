...
const buildSystemPrompt = (context: ReplyContext) => {
  const tone = platformTone[context.platform];
  const salesCTA =
    context.intentCategory === 'Lead Inquiry' || context.intentCategory === 'Demo Booking'
      ? 'Close with a confident CTA like "Would you like to book a quick demo?"'
      : '';
  const supportLine =
    context.intentCategory === 'Support'
      ? 'Be empathetic, reassure the user, and outline clear next steps.'
      : '';
  const fallbackLine =
    'If you are unsure, say: "That is a great question! Let me connect you with one of our Dott Media experts for a personalized answer."';
  const knowledgeBlock =
    context.knowledge && context.knowledge.length
      ? `Relevant knowledge:\n${context.knowledge
