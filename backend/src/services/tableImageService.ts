...
const clampText = (value: string, max = 22) => {
  const trimmed = value.trim();
  if (trimmed.length <= max) return trimmed;
  return `${trimmed.slice(0, Math.max(max - 3, 1))}...`;
};

const parseTimestampLabel = (updatedAt?: string) => {
...
export async function renderLeagueTableImage(input: LeagueTableImageInput): Promise<Buffer> {
  const width = 1800;
  const height = 1800;
  ...
  const rowsSvg = rows
    .map((row, index) => {
      ...
      const played =
        typeof row.played === 'number' && Number.isFinite(row.played) && row.played > 0 ? Math.trunc(row.played) : '-';
      return `
  ...
  const svg = `
    <svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="1660" y="1660" font-family="Arial, Helvetica, sans-serif" font-size="32" font-weight="800" fill="#38bdf8" text-anchor="end">${escapeXml(
        cta,
      )}</text>
    </svg>
  `;
...
export async function renderTopScorersImage(input: TopScorersImageInput): Promise<Buffer> {
  ...
  const rowsSvg = rows
    .map((row, index) => {
      ...
      const fill = index % 2 === 0 ? '#141414' : '#1d1d1d';
      const appearances =
        typeof row.appearances === 'number' && Number.isFinite(row.appearances) && row.appearances > 0
      ...
      return `
        ...
        <text x="980" y="${y + 42}" font-family="Arial, Helvetica, sans-serif" font-size="24" font-weight="500" fill="#d8d8d8">${escapeXml(
          clampText(row.team, 18),
        )}</text>
        <text x="1240" y="${y + 42}" font-family="Arial, Helvetica, sans-serif" font-size="30" font-weight="700" fill="${ACCENT}" text-anchor="end">${Math.trunc(
          row.goals,
        )}</text>
  ...
  const svg = `
    <svg width="${WIDTH}" height="${HEIGHT}" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="120" y="${headerY}" font-family="Arial, Helvetica, sans-serif" font-size="54" font-weight="800" fill="${ACCENT}">${escapeXml(
        `${league.toUpperCase()} TOP SCORERS`,
      )}</text>
      <text x="120" y="${headerY + 42}" font-family="Arial, Helvetica, sans-serif" font-size="24" font-weight="500" fill="#f2f2f2">LIVE RACE - Updated ${escapeXml(
        updatedLabel,
      )} EAT</text>
      ...
      <text x="1480" y="828" font-family="Arial, Helvetica, sans-serif" font-size="24" font-weight="700" fill="${ACCENT}" text-anchor="end">${escapeXml(
        cta,
      )}</text>
    </svg>
  `;
...
export async function renderPredictionsImage(input: PredictionsImageInput): Promise<Buffer> {
  const rows = (input.rows ?? []).slice(0, 8);
  ...
  const rowsSvg = rows
    .map((row, index) => {
      ...
      return `
        ...
        <text x="1330" y="${y + 39}" font-family="Arial, Helvetica, sans-serif" font-size="21" font-weight="800" fill="#111111" text-anchor="middle">${escapeXml(
          clampText(odds, 16),
        )}</text>
      `;
    })
  ...
  const svg = `
    <svg width="${WIDTH}" height="${HEIGHT}" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="120" y="${headerY + 42}" font-family="Arial, Helvetica, sans-serif" font-size="24" font-weight="500" fill="#f2f2f2">PREDICTIONS BOARD - Updated ${escapeXml(
        updatedLabel,
      )} EAT</text>
