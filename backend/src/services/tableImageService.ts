...
async function addLogoOverlay(buffer: Buffer, logTag: string): Promise<Buffer> {
...
export async function renderLeagueTableImage(input: LeagueTableImageInput): Promise<Buffer> {
  ...
  const svg = `
    <svg width="${WIDTH}" height="${HEIGHT}" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="1480" y="828" font-family="Arial, Helvetica, sans-serif" font-size="24" font-weight="700" fill="#ffca08" text-anchor="end">${escapeXml(
        cta,
      )}</text>
    </svg>
  `;
...
export async function renderTopScorersImage(input: TopScorersImageInput): Promise<Buffer> {
  ...
  const rowsSvg = rows
    .map((row, index) => {
      ...
      return `
        ...
        <text x="1240" y="${y + 42}" font-family="Arial, Helvetica, sans-serif" font-size="30" font-weight="700" fill="#ffca08" text-anchor="end">${Math.trunc(
          row.goals,
        )}</text>
  ...
  const svg = `
    <svg width="${WIDTH}" height="${HEIGHT}" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="120" y="${headerY}" font-family="Arial, Helvetica, sans-serif" font-size="54" font-weight="800" fill="#ffca08">${escapeXml(
