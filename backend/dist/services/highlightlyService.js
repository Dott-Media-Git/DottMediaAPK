...
const getConfiguredApiKey = () => process.env.HIGHLIGHTLY_API_KEY?.trim() ||
    process.env.RAPIDAPI_KEY?.trim() ||
    process.env.RAPIDAPI_TOKEN?.trim() ||
    '';
const getApiKey = async (secretOwnerId) => {
...
const isAllowedHighlight = (item, allowedSources) => {
    if (item.type.toUpperCase() !== 'VERIFIED')
        return false;
    if (!item.source || !allowedSources.has(item.source))
        return false;
    if (BLOCKED_CHANNEL_PATTERNS.some(pattern => pattern.test(item.channel || '')))
        return false;
    return true;
};
export const fetchHighlightlyFootballHighlights = async (options) => {
    ...
    for (const date of dates) {
        try {
            const response = await axios.get(`${HIGHLIGHTLY_BASE_URL}/football/highlights`, {
                headers,
                params: {
                    date,
            ...
            for (const row of rows) {
                ...
                if (!normalized || !isAllowedHighlight(normalized, allowedSources))
                    continue;
