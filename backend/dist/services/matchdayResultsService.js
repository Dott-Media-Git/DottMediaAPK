...
async function renderMatchdayImage({ leagueId, date, title, theme, backgroundImage, }) {
    ...
    await new Promise((resolve, reject) => {
        ...
        if (backgroundImage)
            args.push('--background-image', backgroundImage);
...
export async function scheduleMatchdayTables({ date, startAt, spacingHours, }) {
    const schedules = [];
    for (const league of LEAGUES) {
        if (await hasPostedMatchday(league.id, date))
            continue;
        try {
            const themeMap = {
                'uefa.champions': {
    ...
    for (const schedule of schedules) {
        for (const platform of activePlatforms) {
            posts.push({
                ...
                hashtags: 'MatchdayResults, Football, Soccer, FootballResults',
