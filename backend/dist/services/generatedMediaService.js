...
const buildPublicUrl = (subdir, filename) => `${publicBaseUrl()}/public/generated-media/${subdir}/${filename}`;
...
export const saveGeneratedImageBuffer = async (buffer, extension = 'png') => {
...
export const saveGeneratedVideoFile = async (sourcePath, extension = 'mp4') => {
    const filename = `${crypto.randomUUID()}.${extension.replace(/^\./, '')}`;
...
const normalizeExtension = (extension, fallback) => extension.replace(/^\./, '').trim() || fallback;
export const saveUploadedMediaBuffer = async (buffer, type, extension) => {
