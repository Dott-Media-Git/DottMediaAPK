...
import { AnalyticsService, getActivityHeatmap, getEngagementStats, getFollowupStats, getInboundStats, getOutboundStats, getWebLeadStats, getWebTrafficStats, } from './analyticsService.js';
import { SocialAnalyticsService } from '../packages/services/socialAnalyticsService.js';
import { AssistantStrategyService } from './assistantStrategyService.js';
import { KnowledgeBaseService } from './knowledgeBaseService.js';
import { getLiveSocialMetrics } from './liveSocialMetricsService.js';
...
const extractOpenAIError = (error) => {
    const err = error;
...
const buildAssistantErrorText = (kind) => {
    if (kind === 'billing') {
...
const LOCALE_RESPONSE_LANGUAGE = {
    en: 'English',
...
const emptyWebLeadStats = () => ({
    leads: 0,
    messages: 0,
    conversionRate: 0,
});
const emptyWebTrafficStats = () => ({
    visitors: 0,
    interactions: 0,
    redirectClicks: 0,
    engagementRate: 0,
...
export class AssistantService {
    shouldProvideWeeklySummary(question) {
        const normalized = question.toLowerCase();
        const weekMention = /\b(week|weekly|this week|last week|semaine|hebdo|cette semaine|semaine derniere|woche|diese woche|letzte woche|woechentlich)\b/.test(normalized);
        const metricMention = /\b(performance|summary|stats|kpi|metrics|engagement|leads|conversions|resume|statistiques|metriques|leistung|zusammenfassung|statistiken|kennzahlen|metriken|konversionen)\b/.test(normalized);
        const zhWeekMention = /本周|上周|每周|周报|本星期|上星期/.test(normalized);
        const zhMetricMention = /绩效|表现|统计|指标|数据|参与度|线索|转化/.test(normalized);
        const arWeekMention = /اسبوع|أسبوع|الاسبوع|الأسبوع|هذا الاسبوع|هذا الأسبوع|الاسبوع الماضي|الأسبوع الماضي/.test(normalized);
        const arMetricMention = /اداء|الأداء|مؤشر|مؤشرات|احصائيات|احصاء|مقاييس|تفاعل|عملاء محتملين|تحويلات/.test(normalized);
        const esWeekMention = /semana|semanal|esta semana|la semana pasada/.test(normalized);
        const esMetricMention = /rendimiento|resumen|estadisticas|estadísticas|kpi|metricas|métricas|engagement|interaccion|interacción|leads|conversiones/.test(normalized);
        const ptWeekMention = /semana|semanal|esta semana|semana passada/.test(normalized);
        const ptMetricMention = /desempenho|resumo|estatisticas|estatísticas|kpi|metricas|métricas|engajamento|leads|conversoes|conversões/.test(normalized);
        const idWeekMention = /minggu|minggu ini|minggu lalu|mingguan/.test(normalized);
        const idMetricMention = /kinerja|ringkasan|statistik|kpi|metrik|engagement|interaksi|leads|konversi/.test(normalized);
        const jaWeekMention = /今週|先週|週間/.test(normalized);
        const jaMetricMention = /パフォーマンス|概要|統計|kpi|指標|エンゲージメント|リード|コンバージョン/.test(normalized);
        const ruWeekMention = /недел|на этой неделе|прошлой неделе|еженедельно/.test(normalized);
        const ruMetricMention = /производительност|сводка|статистик|kpi|метрик|вовлеченност|лид|конверс/.test(normalized);
    ...
    shouldDraftStrategy(question) {
        const normalized = question.toLowerCase();
        return (/\b(strategy|growth strategy|marketing plan|growth plan|campaign plan|strategy plan|action plan|go to market|g2m)\b/.test(normalized) ||
    ...
    extractStrategyId(question) {
        const match = question.match(/strat-[a-z0-9]{6}/i);
        return match ? match[0].toUpperCase() : undefined;
    }
    shouldSendMonthlyReport(question) {
        const normalized = question.toLowerCase();
    ...
    extractEmailAddresses(question) {
        const emails = Array.from(new Set((question.match(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi) || []).map(email => email.toLowerCase())));
        return emails;
    }
    normalizeAssistantTone(value) {
    ...
    normalizeAssistantVoice(value) {
        const normalized = `${value ?? ''}`.trim().toLowerCase();
        if (normalized.includes('female') || normalized.includes('woman'))
            return 'female';
        if (normalized.includes('male') || normalized.includes('man'))
            return 'male';
        if (normalized.includes('any') || normalized.includes('neutral'))
            return 'neutral';
        return '';
    }
    buildPersonalityPrompt(context) {
    ...
    resolveLocale(value) {
        if (value && LOCALE_RESPONSE_LANGUAGE[value])
            return value;
        return 'en';
    }
    ...
    formatDelta(label, current, previous, unit, locale) {
    ...
    isBettingBrand(snapshot) {
        const haystack = `${snapshot.company ?? ''} ${snapshot.businessGoals ?? ''}`.toLowerCase();
        return haystack.includes('bwin') || haystack.includes('bet');
    }
    getConversionLabel(snapshot) {
        return this.isBettingBrand(snapshot) ? 'bet button clicks' : 'conversions';
    }
    async safeResolve(label, action, fallback) {
        try {
            return await action();
        }
        catch (error) {
            console.warn(`[assistant] failed to load ${label}`, error.message);
            return fallback;
        }
    }
    resolveConnectedChannels(userData, fallback = []) {
    ...
    buildPlatformHighlights(snapshot) {
        ...
        return platforms
            .map(platform => `${platform.name}: ${this.formatWholeNumber(platform.views)} views, ${this.formatWholeNumber(platform.interactions)} interactions, ${this.formatWholeNumber(platform.conversions)} ${this.getConversionLabel(snapshot)}`)
            .join('; ');
    }
    buildPostingActivitySummary(rows) {
        if (!rows.length) {
        ...
        const totals = rows.reduce((acc, row) => {
            ...
            const perPlatform = (row.perPlatform ?? {});
        ...
        const topPlatforms = Object.entries(totals.platformCounts)
            ...
            .join(', ');
        return `Posting in the last 7 days: ${this.formatWholeNumber(totals.posted)} posted, ${this.formatWholeNumber(totals.failed)} failed, ${this.formatWholeNumber(totals.skipped)} skipped.${topPlatforms ? ` Top channels: ${topPlatforms}.` : ''}`;
    }
    buildDailyReviewSummary(snapshot) {
        const todayKey = new Date().toISOString().slice(0, 10);
        const todayRow = snapshot.activityHeatmap.find(row => row.date === todayKey);
        if (!todayRow) {
            return 'Today has no recorded activity yet.';
        }
        return `Today so far: ${this.formatWholeNumber(todayRow.views)} views, ${this.formatWholeNumber(todayRow.interactions)} interactions, ${this.formatWholeNumber(todayRow.outbound)} outbound actions, ${this.formatWholeNumber(todayRow.conversions)} ${this.getConversionLabel(snapshot)}.`;
    }
    buildWeeklyPerformanceSummary(snapshot) {
        ...
        return `This week so far: ${this.formatWholeNumber(totals.views)} views, ${this.formatWholeNumber(totals.interactions)} interactions, ${this.formatWholeNumber(totals.outbound)} outbound actions, ${this.formatWholeNumber(totals.conversions)} ${this.getConversionLabel(snapshot)}.`;
    }
    buildMetricInsight(snapshot, metric) {
        const conversionLabel = this.getConversionLabel(snapshot);
        switch (metric) {
            case 'views':
                return `Live views across connected channels are ${this.formatWholeNumber(snapshot.liveSocial.summary.views)} over the last ${snapshot.liveSocial.lookbackHours} hours. ${this.buildPlatformHighlights(snapshot)}`;
            case 'interactions':
                return `Live interactions are ${this.formatWholeNumber(snapshot.liveSocial.summary.interactions)} with an engagement rate of ${snapshot.liveSocial.summary.engagementRate.toFixed(2)}%. ${this.buildPlatformHighlights(snapshot)}`;
            case 'outbound':
                return `Outbound activity has sent ${this.formatWholeNumber(snapshot.outbound.prospectsContacted)} messages, produced ${this.formatWholeNumber(snapshot.outbound.replies)} replies, and ${this.formatWholeNumber(snapshot.outbound.conversions)} conversions.`;
            case 'conversions':
                return `Current ${conversionLabel} are ${this.formatWholeNumber(snapshot.liveSocial.summary.conversions || snapshot.webTraffic.redirectClicks || snapshot.outbound.conversions)}. Web redirect clicks are ${this.formatWholeNumber(snapshot.webTraffic.redirectClicks)} and outbound conversions are ${this.formatWholeNumber(snapshot.outbound.conversions)}.`;
            default:
                return [
                    this.buildDailyReviewSummary(snapshot),
                    this.buildWeeklyPerformanceSummary(snapshot),
                    `Live social summary: ${this.formatWholeNumber(snapshot.liveSocial.summary.views)} views, ${this.formatWholeNumber(snapshot.liveSocial.summary.interactions)} interactions, ${this.formatWholeNumber(snapshot.liveSocial.summary.conversions)} ${conversionLabel}.`,
                    `Outbound: ${this.formatWholeNumber(snapshot.outbound.prospectsContacted)} contacted, ${this.formatWholeNumber(snapshot.outbound.replies)} replies, ${this.formatWholeNumber(snapshot.outbound.conversions)} conversions.`,
                ].join(' ');
        }
    }
    async loadAccountSnapshot(context) {
        if (!context.userId) {
            return null;
        }
        ...
        const crmData = profileData.crmData ?? {};
        const userData = userSnap?.data() ?? {};
        const scopeId = context.orgId?.trim() || `${crmData.orgId ?? ''}`.trim() || `${userData.orgId ?? ''}`.trim() || undefined;
        const analyticsScope = {
            userId: context.userId,
            scopeId,
        };
        const [analyticsSummary, liveSocial, outbound, inbound, engagement, followups, webLeads, webTraffic, activityHeatmap, socialDaily,] = await Promise.all([
            ...
            this.safeResolve('activity heatmap', () => getActivityHeatmap(analyticsScope, 14), []),
            this.safeResolve('social daily', () => socialAnalyticsService.getDailySummary(context.userId, 7), []),
        ]);
        return {
    ...
    buildAccountContextBlock(snapshot) {
        const conversionLabel = this.getConversionLabel(snapshot);
        ...
        return [
            ...
            this.buildDailyReviewSummary(snapshot),
            this.buildWeeklyPerformanceSummary(snapshot),
            `Live social performance (last ${snapshot.liveSocial.lookbackHours}h): ${this.formatWholeNumber(snapshot.liveSocial.summary.views)} views, ${this.formatWholeNumber(snapshot.liveSocial.summary.interactions)} interactions, engagement rate ${snapshot.liveSocial.summary.engagementRate.toFixed(2)}%, ${this.formatWholeNumber(snapshot.liveSocial.summary.conversions)} ${conversionLabel}.`,
            `Top platform view: ${this.buildPlatformHighlights(snapshot)}`,
            `Outbound engine: ${this.formatWholeNumber(snapshot.outbound.prospectsContacted)} contacted, ${this.formatWholeNumber(snapshot.outbound.replies)} replies, ${this.formatWholeNumber(snapshot.outbound.positiveReplies)} positive replies, ${this.formatWholeNumber(snapshot.outbound.conversions)} conversions, ${this.formatWholeNumber(snapshot.outbound.demoBookings)} demos booked.`,
            `Inbound and engagement: ${this.formatWholeNumber(snapshot.inbound.messages)} inbound messages, ${this.formatWholeNumber(snapshot.inbound.leads)} qualified leads, ${this.formatWholeNumber(snapshot.engagement.comments)} comments, ${this.formatWholeNumber(snapshot.engagement.replies)} replies sent, ${this.formatWholeNumber(snapshot.engagement.conversions)} engagement conversions.`,
            `Follow-ups and web: ${this.formatWholeNumber(snapshot.followups.sent)} follow-ups sent, ${this.formatWholeNumber(snapshot.followups.replies)} replies, ${this.formatWholeNumber(snapshot.webTraffic.visitors)} website visitors, ${this.formatWholeNumber(snapshot.webTraffic.redirectClicks)} web redirect clicks, ${this.formatWholeNumber(snapshot.webLeads.leads)} web leads.`,
            this.buildPostingActivitySummary(snapshot.socialDaily),
            `Automation/job status: active ${this.formatWholeNumber(snapshot.analyticsSummary.jobBreakdown.active)}, queued ${this.formatWholeNumber(snapshot.analyticsSummary.jobBreakdown.queued)}, failed ${this.formatWholeNumber(snapshot.analyticsSummary.jobBreakdown.failed)}. Recent jobs: ${automationLine}.`,
        ]
            .filter(Boolean)
            .join('\n');
    }
    async buildWeeklySummary(userId, locale) {
        const copy = locale === 'fr'
            ? {
                weekSoFar: (current) => `Cette semaine jusqu ici : engagement ${current.engagement}%, leads ${current.leads}, conversions ${current.conversions}.`,
                weeklyPerformance: (engagementLine, leadsLine, conversionsLine) => `Performance hebdomadaire : ${engagementLine}. ${leadsLine}. ${conversionsLine}.`,
                socialThisWeek: (posted, failed, skipped) => `Activite sociale cette semaine : ${posted} posts publies, ${failed} en echec, ${skipped} ignores.`,
                socialActivity: (postedLine, failedLine) => `Activite sociale : ${postedLine}. ${failedLine}.`,
                noActivity: 'Pas d activite en direct cette semaine. Une fois les posts ou automations executes, je resumerai la performance ici.',
            ...
            : locale === 'de'
                ? {
                    weekSoFar: (current) => `Diese Woche bisher: Engagement ${current.engagement}%, Leads ${current.leads}, Conversions ${current.conversions}.`,
                    weeklyPerformance: (engagementLine, leadsLine, conversionsLine) => `Wochenleistung: ${engagementLine}. ${leadsLine}. ${conversionsLine}.`,
                    socialThisWeek: (posted, failed, skipped) => `Social-Aktivitat diese Woche: ${posted} Posts veroffentlicht, ${failed} fehlgeschlagen, ${skipped} ubersprungen.`,
                    socialActivity: (postedLine, failedLine) => `Social-Aktivitat: ${postedLine}. ${failedLine}.`,
                    noActivity: 'Diese Woche noch keine Live-Aktivitat. Sobald Posts oder Automationen laufen, fasse ich die Performance hier zusammen.',
                ...
                : locale === 'es'
                    ? {
                        weekSoFar: (current) => `Esta semana hasta ahora: engagement ${current.engagement}%, leads ${current.leads}, conversiones ${current.conversions}.`,
                        weeklyPerformance: (engagementLine, leadsLine, conversionsLine) => `Rendimiento semanal: ${engagementLine}. ${leadsLine}. ${conversionsLine}.`,
                        socialThisWeek: (posted, failed, skipped) => `Actividad social esta semana: ${posted} publicaciones, ${failed} fallidas, ${skipped} omitidas.`,
                        socialActivity: (postedLine, failedLine) => `Actividad social: ${postedLine}. ${failedLine}.`,
                        noActivity: 'Sin actividad en vivo esta semana. Cuando se publiquen posts o automaciones, resumiré el rendimiento aquí.',
                    ...
                    : locale === 'pt'
                        ? {
                            weekSoFar: (current) => `Esta semana até agora: engajamento ${current.engagement}%, leads ${current.leads}, conversões ${current.conversions}.`,
                            weeklyPerformance: (engagementLine, leadsLine, conversionsLine) => `Performance semanal: ${engagementLine}. ${leadsLine}. ${conversionsLine}.`,
                            socialThisWeek: (posted, failed, skipped) => `Atividade social nesta semana: ${posted} posts publicados, ${failed} com falha, ${skipped} ignorados.`,
                            socialActivity: (postedLine, failedLine) => `Atividade social: ${postedLine}. ${failedLine}.`,
                            noActivity: 'Sem atividade ao vivo nesta semana. Quando posts ou automações rodarem, resumo a performance aqui.',
                        ...
                        : locale === 'id'
                            ? {
                                weekSoFar: (current) => `Minggu ini sejauh ini: engagement ${current.engagement}%, leads ${current.leads}, konversi ${current.conversions}.`,
                                weeklyPerformance: (engagementLine, leadsLine, conversionsLine) => `Performa mingguan: ${engagementLine}. ${leadsLine}. ${conversionsLine}.`,
                                socialThisWeek: (posted, failed, skipped) => `Aktivitas sosial minggu ini: ${posted} posting, ${failed} gagal, ${skipped} dilewati.`,
                                socialActivity: (postedLine, failedLine) => `Aktivitas sosial: ${postedLine}. ${failedLine}.`,
                                noActivity: 'Belum ada aktivitas live minggu ini. Setelah posting atau automasi berjalan, saya rangkum performa di sini.',
                            ...
                            : locale === 'ja'
                                ? {
                                    weekSoFar: (current) => `今週これまで: エンゲージメント ${current.engagement}%、リード ${current.leads}、コンバージョン ${current.conversions}。`,
                                    weeklyPerformance: (engagementLine, leadsLine, conversionsLine) => `週次パフォーマンス: ${engagementLine}。${leadsLine}。${conversionsLine}。`,
                                    socialThisWeek: (posted, failed, skipped) => `今週のソーシャル活動: 投稿 ${posted}件、失敗 ${failed}件、スキップ ${skipped}件。`,
                                    socialActivity: (postedLine, failedLine) => `ソーシャル活動: ${postedLine}。${failedLine}。`,
                                    noActivity: '今週はライブ活動がありません。投稿や自動化が実行されたら、ここで要約します。',
                                ...
                                : locale === 'ru'
                                    ? {
                                        weekSoFar: (current) => `На этой неделе: вовлеченность ${current.engagement}%, лиды ${current.leads}, конверсии ${current.conversions}.`,
                                        weeklyPerformance: (engagementLine, leadsLine, conversionsLine) => `Недельная эффективность: ${engagementLine}. ${leadsLine}. ${conversionsLine}.`,
                                        socialThisWeek: (posted, failed, skipped) => `Социальная активность на этой неделе: ${posted} публикаций, ${failed} сбоев, ${skipped} пропущено.`,
                                        socialActivity: (postedLine, failedLine) => `Социальная активность: ${postedLine}. ${failedLine}.`,
                                        noActivity: 'На этой неделе нет активности вживую. Когда посты или автоматизации запустятся, я резюмирую здесь.',
                                    ...
                                    : locale === 'ko'
                                        ? {
                                            weekSoFar: (current) => `이번 주 현재: 참여도 ${current.engagement}%, 리드 ${current.leads}, 전환 ${current.conversions}.`,
                                            weeklyPerformance: (engagementLine, leadsLine, conversionsLine) => `주간 성과: ${engagementLine}. ${leadsLine}. ${conversionsLine}.`,
                                            socialThisWeek: (posted, failed, skipped) => `이번 주 소셜 활동: 게시물 ${posted}건, 실패 ${failed}건, 건너뜀 ${skipped}건.`,
                                            socialActivity: (postedLine, failedLine) => `소셜 활동: ${postedLine}. ${failedLine}.`,
                                            noActivity: '이번 주 라이브 활동이 없습니다. 게시물이나 자동화가 실행되면 여기서 요약합니다.',
                                        ...
                                        : locale === 'ar'
                                            ? {
                                                weekSoFar: (current) => `هذا الاسبوع حتى الان: التفاعل ${current.engagement}%, العملاء المحتملون ${current.leads}, التحويلات ${current.conversions}.`,
                                                weeklyPerformance: (engagementLine, leadsLine, conversionsLine) => `اداء الاسبوع: ${engagementLine}. ${leadsLine}. ${conversionsLine}.`,
                                                socialThisWeek: (posted, failed, skipped) => `النشاط الاجتماعي هذا الاسبوع: ${posted} منشور، ${failed} فشل، ${skipped} تم تخطيه.`,
                                                socialActivity: (postedLine, failedLine) => `النشاط الاجتماعي: ${postedLine}. ${failedLine}.`,
                                            ...
                                            : locale === 'zh'
                                                ? {
                                                    weekSoFar: (current) => `本周截至目前：互动率 ${current.engagement}%，线索 ${current.leads}，转化 ${current.conversions}。`,
                                                    weeklyPerformance: (engagementLine, leadsLine, conversionsLine) => `周度表现：${engagementLine}。${leadsLine}。${conversionsLine}。`,
                                                    socialThisWeek: (posted, failed, skipped) => `本周社媒活动：发布 ${posted} 条，失败 ${failed} 条，跳过 ${skipped} 条。`,
                                                    socialActivity: (postedLine, failedLine) => `社媒活动：${postedLine}。${failedLine}。`,
                                                ...
                                                : {
                                                    weekSoFar: (current) => `This week so far: engagement ${current.engagement}%, leads ${current.leads}, conversions ${current.conversions}.`,
                                                    weeklyPerformance: (engagementLine, leadsLine, conversionsLine) => `Weekly performance: ${engagementLine}. ${leadsLine}. ${conversionsLine}.`,
                                                    socialThisWeek: (posted, failed, skipped) => `Social activity this week: ${posted} posts published, ${failed} failed, ${skipped} skipped.`,
                                                    socialActivity: (postedLine, failedLine) => `Social activity: ${postedLine}. ${failedLine}.`,
                                                    noActivity: 'No live activity yet this week. Once posts or automations run, I will summarize performance here.',
        ...
        const analyticsLine = hasAnalyticsData
            ? (() => {
                ...
                const engagementLine = this.formatDelta(copy.labels.engagement, current.engagement, previous.engagement, '%', locale);
            ...
            : null;
        const socialSorted = [...(socialRows ?? [])].sort((a, b) => `${a?.date ?? ''}`.localeCompare(`${b?.date ?? ''}`));
        const socialLast7 = socialSorted.slice(-7);
        const socialPrev7 = socialSorted.slice(-14, -7);
        const sumSocial = (rows, key) => rows.reduce((sum, row) => sum + Number(row[key] ?? 0), 0);
        const socialCurrent = {
        ...
        const hasSocialData = socialCurrent.attempted + socialCurrent.posted + socialCurrent.failed + socialCurrent.skipped > 0 ||
        ...
        const socialLine = hasSocialData
            ...
            : null;
        const lines = [analyticsLine, socialLine].filter(Boolean);
        if (!lines.length) {
    ...
    async answer(question, context) {
        ...
        try {
            ...
            console.warn('Failed to load knowledge snippets', error.message);
        }
