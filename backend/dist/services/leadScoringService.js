...
export class LeadScoringService {
    scoreLead(input) {
        ...
        if (input.profile.goal)
            score += 8;
        if (input.profile.budget)
            score += 12;
        if (input.intent === 'Lead Inquiry')
            score += 15;
        if (input.intent === 'Demo Booking')
            score += 18;
        ...
        positiveSignals.forEach(signal => {
            if (keywordString.includes(signal.replace(/\s+/g, '')))
                score += 6;
        });
        urgencySignals.forEach(signal => {
            if (input.message.toLowerCase().includes(signal))
                score += 5;
        });
