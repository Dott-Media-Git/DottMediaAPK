...
<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
  ...
  ${headlineLines.map((line, index) => `<text x="120" y="${410 + index * 72}" fill="#ffffff" font-family="Arial Black, Arial, Helvetica, sans-serif" font-size="58" font-weight="900">${escapeSvg(line)}</text>`).join("")}
  ${bodyLines.map((line, index) => `<text x="120" y="${650 + index * 46}" fill="#dff8e7" font-family="Arial, Helvetica, sans-serif" font-size="34" font-weight="700">${escapeSvg(line)}</text>`).join("")}
  <rect x="120" y="800" width="820" height="150" rx="28" fill="#e9fff0" opacity="0.96"/>
  ${reasonLines.slice(0, 3).map((line, index) => `<text x="155" y="${842 + index * 38}" fill="#082019" font-family="Arial, Helvetica, sans-serif" font-size="28" font-weight="800">${escapeSvg(line)}</text>`).join("")}
...
async function renderDottEnergyEducationCard(topic) {
  ...
  }).composite(composites).jpeg({ quality: 92, mozjpeg: true }).toBuffer();
