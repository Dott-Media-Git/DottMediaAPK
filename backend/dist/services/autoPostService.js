...
import { buildDottEnergyEducationCaption, buildDottEnergyProductCaption, dottEnergyEducationHistoryKey, dottEnergyProductHistoryKey, pickDottEnergyEducationTopic, pickDottEnergyProduct, renderDottEnergyEducationCard, renderDottEnergyProductImage, } from './dottEnergyProductService.js';
...
export class AutoPostService {
    ...
    async loadAutopostJob(userId) {
        ...
        try {
            ...
            const snap = await this.withTimeout(autopostCollection.doc(userId).get(), firestoreTimeoutMs, 'firestore_autopost_job_fetch');
            if (snap.exists) {
                const job = snap.data();
                this.cacheJob(userId, job);
                return job;
    ...
    async runBwinEmergencyPost(now) {
        ...
        try {
            ...
            const persistentRecentKeys = new Set();
            if (emergencyOwnerId) {
                try {
                    ...
                    for (const post of recentPosts) {
                        ...
                        const urls = Array.from(caption.matchAll(/https?:\/\/\S+/g)).map(match => match[0].replace(/[),.;]+$/, ''));
            ...
            for (const candidate of newsCandidates) {
                ...
                if (this.isBlockedBwinNewsItem(item))
                    continue;
                ...
                if (key && key === this.emergencyXLastKey)
                    continue;
                if (scheduledPostIds.some(postId => persistentRecentKeys.has(postId)))
                    continue;
            ...
            if (posted) {
                ...
                if (emergencyOwnerId && selectedScheduledPostId) {
                    await Promise.all(results
                        ...
                        .map(result => supabaseFallbackService.addSocialLog({
                        userId: emergencyOwnerId,
    ...
    async runDueJobs() {
        ...
        try {
            ...
            const firestoreTimeoutMs = Math.max(Number(process.env.AUTOPOST_FIRESTORE_TIMEOUT_MS ?? 15000), 3000);
            const [standardSnap, reelsSnap, storiesSnap, trendSnap, missingReelsSnap, missingStoriesSnap] = await this.withTimeout(Promise.all([
            ...
            ]), firestoreTimeoutMs, 'firestore_due_jobs_query');
    ...
    hasCredentialsForPlatform(platform, credentials) {
        if (platform === 'facebook' || platform === 'facebook_story')
            return Boolean(credentials.facebook);
        if (platform === 'instagram' || platform === 'instagram_story' || platform === 'instagram_reels') {
            return Boolean(credentials.instagram);
        }
        if (platform === 'threads')
            return Boolean(credentials.threads);
        if (platform === 'linkedin')
            return Boolean(credentials.linkedin);
        if (platform === 'twitter' || platform === 'x')
            return Boolean(credentials.twitter);
        if (platform === 'tiktok')
            return Boolean(credentials.tiktok);
        if (platform === 'youtube')
            return Boolean(credentials.youtube);
        return true;
    }
    ...
    getInstagramAttemptField(platform) {
        if (platform === 'instagram')
            return 'instagramFeedLastAttemptAt';
        if (platform === 'instagram_story')
            return 'instagramStoryLastAttemptAt';
        return null;
    }
    ...
    shouldSkipNicheInstagramAttempt(userId, job, platform, now = Date.now()) {
        ...
        if (!lastAttempt)
            return false;
        const cooldownHours = platform === 'instagram_story'
            ? this.getStoryIntervalHours(userId, job.storyIntervalHours)
            : this.getFeedIntervalHours(userId, job.intervalHours);
    ...
    isBlockedBwinNewsItem(item) {
        ...
        if (!link || !title)
            return true;
        if (link.includes('bbc.co.uk/sounds') ||
    ...
    async overlayNewsHeadline(source, headline) {
        ...
        const headlineSvg = lines
            .map((line, index) => `<text x="96" y="${textY + index * lineGap}" font-family="Arial Black, Arial, Helvetica, sans-serif" font-size="92" font-weight="900" fill="#ffffff">${this.escapeSvgText(line.toUpperCase())}</text>`)
            .join('\n');
        const svg = `
    ...
    async generateBwinSportsFallbackImage(headline, subline) {
        ...
        const headlineSvg = headlineLines
            .map((line, index) => `<text x="110" y="${410 + index * 122}" font-family="Arial, Helvetica, sans-serif" font-size="102" font-weight="800" fill="#f8fafc">${this.escapeSvgText(line)}</text>`)
            .join('');
        const sublineSvg = sublineLines
            .map((line, index) => `<text x="110" y="${980 + index * 62}" font-family="Arial, Helvetica, sans-serif" font-size="48" font-weight="500" fill="#cbd5e1">${this.escapeSvgText(line)}</text>`)
            .join('');
        const svg = `
      <svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
        <defs>
    ...
    async executeTrendStories(userId, job) {
        ...
        if (this.isBwinScopeUser(userId)) {
            ...
            const newsCandidates = this.filterBwinNewsCandidates(candidates);
            const picked = this.pickFreshNewsCandidate(newsCandidates, new Set(this.getTrendRecentKeys(job)))?.candidate ??
                newsCandidates[0] ??
                null;
        ...
        if (scope === 'football') {
            try {
                ...
                const top = selectedContentType === 'video'
                    ? footballCandidates.find(candidate => (candidate.items ?? []).some(item => Boolean(item.videoUrl?.trim()))) ?? footballCandidates[0]
            ...
            if (!imageUrls.length) {
                ...
                for (const candidate of footballCandidates.slice(0, 8)) {
                    for (const item of (candidate.items ?? []).slice(0, 3)) {
                        const resolved = await this.resolveBestNewsImageUrl(item.imageUrl?.trim(), item.link?.trim());
                        if (resolved)
                            backupImages.push(resolved);
                        if (backupImages.length >= 3)
                            break;
                    }
                    if (backupImages.length >= 3)
                        break;
    ...
    async executeJob(userId, job, options = {}) {
        ...
        const configuredIntervalHours = options.intervalHours ??
            (isReelsRun
        ...
        const platforms = requestedPlatforms.filter(platform => !this.shouldSkipNicheInstagramAttempt(userId, job, platform));
        const instagramAttemptFields = new Set(platforms
        ...
        if (this.isBwinScopeUser(userId) && !isReelsRun && !isStoryRun) {
            const feedPlatforms = platforms.filter(platform => ['facebook', 'instagram', 'threads', 'x', 'twitter'].includes(platform));
        ...
        if (clientPhotoProfile?.key === 'dottenergy') {
            ...
            try {
                ...
                const shouldPostStoreProduct = isStoryRun ||
                ...
                if (shouldPostStoreProduct) {
                    ...
                    const coverImageUrl = await renderDottEnergyProductImage(product, product.images[0], isStoryRun ? 'story' : 'feed');
        ...
        for (const platform of publishPlatforms) {
            ...
            let signature = caption === captionSelection.caption
            ...
            try {
                ...
                if (['facebook', 'instagram', 'facebook_story', 'instagram_story'].includes(platform) &&
                    response?.remoteId) {
        ...
        const nextRunDate = new Date(Date.now() + effectiveIntervalHours * 60 * 60 * 1000);
        const nextRecentImages = this.mergeRecentImages(recentImages, [
            ...imageUrls,
        ...
        const nextRecentCaptions = this.mergeRecentCaptions(recentCaptions, [
            ...usedCaptions,
            usedBeforwardStockKey,
            usedStaysphereListingKey,
            usedGamersSteamKey,
        ...
        if (clientPhotoProfile?.key === 'dottenergy' && !isReelsRun && !isStoryRun) {
            updatePayload.dottEnergyProductNextRun = admin.firestore.Timestamp.fromDate(new Date(Date.now() +
        ...
        if (clientPhotoProfile?.key === 'dottenergy' && !isReelsRun && !isStoryRun) {
            nextRecord.dottEnergyProductNextRun = admin.firestore.Timestamp.fromDate(new Date(Date.now() +
                (dottEnergyPostedStoreProduct ? DOTT_ENERGY_PRODUCT_INTERVAL_HOURS : 1) * 60 * 60 * 1000));
            if (dottEnergyPostedStoreProduct) {
    ...
    extractContentKeys(value) {
        ...
        for (const match of value.matchAll(/https?:\/\/[^\s)]+/gi)) {
            ...
            if (/dott-energy-2\.myshopify\.com\/products\/([^/?#]+)/i.test(normalized)) {
                ...
                if (handle)
                    keys.push(`dott-energy-product:${handle.toLowerCase()}`);
            }
        }
        return keys;
    }
