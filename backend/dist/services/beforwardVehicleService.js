...
const USER_AGENT = process.env.BEFORWARD_USER_AGENT ||
    'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0 Safari/537.36';
const httpsAgent = process.env.BEFORWARD_TLS_INSECURE === 'false' ? undefined : new https.Agent({ rejectUnauthorized: false });
...
const estimateUgxFromUsd = (priceUsd) => {
...
async function uploadCarmarketImage(buffer, folder = 'covers') {
...
const extractCarbarnEmbeddedImages = (html, title) => {
    const escapedTitle = escapeRegExp(title).replace(/\s+/g, '\\s+');
    const block = html.match(new RegExp(`\\\\\"title\\\\\":\\\\\"${escapedTitle}\\\\\"[\\s\\S]{0,14000}?\\\\\"exteriorPhoto\\\\\":\\[(.*?)\\]`, 'i'))?.[1];
    if (!block)
        return [];
    return unique(Array.from(block.matchAll(/\\\"(https?:[^\\\"]+?\.(?:jpg|jpeg|png|webp))\\\"/gi)).map(match => match[1].replace(/\\\//g, '/')));
};
const parseCarbarnEmbeddedVehicles = (html) => {
    const vehicles = [];
    const vehiclePattern = /\\"title\\":\\"([^\\"]+)\\"[\s\S]{0,1600}?\\"slug\\":\\"([^\\"]+)\\"[\s\S]{0,1600}?\\"stockNo\\":\\"([^\\"]+)\\"[\s\S]{0,10000}?\\"exteriorPhoto\\":\[(.*?)\][\s\S]{0,8000}?\\"UG\\":\{(.*?)\}/gi;
    for (const match of html.matchAll(vehiclePattern)) {
        const [, rawTitle, rawSlug, rawStockNo, photoBlock, priceBlock] = match;
        const images = unique(Array.from(photoBlock.matchAll(/\\"(https?:[^\\"]+?\.(?:jpg|jpeg|png|webp))\\"/gi)).map(imageMatch => imageMatch[1].replace(/\\\//g, '/')));
        if (!images.length)
            continue;
...
export async function fetchBeforwardVehicle(url) {
    const vehicleUrl = normalizeUrl(url);
    const html = await fetchHtml(vehicleUrl);
    const $ = cheerio.load(html);
    const title = $('meta[property="og:title"]').attr('content')?.replace(/^BE FORWARD\s*:\s*/i, '').trim() ||
        $('title').text().replace(/\s*-\s*BE FORWARD.*$/i, '').trim() ||
        'BE FORWARD vehicle';
    const stockNo = $('script[type="application/ld+json"]')
        .text()
        .match(/"sku"\s*:\s*"([^"]+)"/i)?.[1]
        ?.trim() ||
        ...
        '';
    const priceUsd = $('script[type="application/ld+json"]')
        .text()
        .match(/"price"\s*:\s*"([^"]+)"/i)?.[1]
        ?.trim() ||
    ...
    const images = unique([
        ...
        ...Array.from(html.matchAll(/\/\/image-cdn\.beforward\.jp\/large\/[^"'\s<>]+/gi)).map(match => normalizeUrl(match[0])),
        normalizeUrl($('meta[property="og:image"]').attr('content') || ''),
    ].map(url => url.replace(/\?w=\d+$/i, ''))).slice(0, 10);
    if (!images.length) {
    ...
    for (const script of scripts) {
        try {
            ...
            for (const entry of items) {
                ...
                if (stockNo && options.recentStockNos?.has(`CARBARN-${stockNo}`))
                    continue;
...
async function fetchMileleVehicle(url) {
    const vehicleUrl = normalizeUrl(url);
    const html = await fetchHtml(vehicleUrl);
    const $ = cheerio.load(html);
    const title = decodeHtmlText($('.stm-listing-single-price-title h1').first().text() ||
        $('h1').first().text() ||
        $('title').text().replace(/\s*-\s*Milele Motors.*$/i, '') ||
        'Milele vehicle');
    const images = unique([
        ...Array.from(html.matchAll(/data-src-img=["']([^"']+?\.(?:jpg|jpeg|png|webp)(?:\?[^"']*)?)["']/gi)).map(match => match[1]),
        ...Array.from(html.matchAll(/https?:\/\/milele\.com\/wp-content\/uploads\/[^"'\s<>]+?\.(?:jpg|jpeg|png|webp)/gi)).map(match => match[0]),
    ].map(url => normalizeUrl(url))).slice(0, 10);
    if (images.length < 2)
        throw new Error('No Milele vehicle gallery images found');
...
async function pickMileleVehicle(options = {}) {
    const html = await fetchHtml('https://milele.com/inventory/');
    const links = unique(Array.from(html.matchAll(/https:\/\/milele\.com\/listings\/[^"'\s<>]+?\//gi)).map(match => match[0]));
    const preferred = [
    ...
    for (const link of unique(preferred)) {
        ...
        if (stockKey && options.recentStockNos?.has(stockKey))
            continue;
        try {
...
export async function pickBeforwardVehicle(options = {}) {
    ...
    const links = unique(Array.from(html.matchAll(/href="(\/[a-z0-9-]+\/[a-z0-9-]+\/[a-z]{2}\d{6}\/id\/\d+\/)"/gi)).map(match => normalizeUrl(match[1])));
    for (const link of links) {
        const stockNo = link.match(/\/([a-z]{2}\d{6})\//i)?.[1]?.toUpperCase();
        if (stockNo && options.recentStockNos?.has(stockNo))
            continue;
        try {
...
export async function renderCarmarketCoverImage(vehicle) {
