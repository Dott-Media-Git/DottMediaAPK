...
import linkedinIntegrationRoutes from './routes/linkedinIntegrationRoutes.js';
import { canUseOutboundPipeline } from './utils/socialAccess.js';
import twitterIntegrationRoutes from './routes/twitterIntegrationRoutes.js';
...
import { checkSupabaseHealth } from './services/supabaseHealthService.js';
...
app.get('/api/migration/supabase/health', async (req, res, next) => {
    try {
        ...
        const providedToken = req.header('x-migration-token') ?? (typeof req.query.token === 'string' ? req.query.token : '');
...
app.use('/', linkedinIntegrationRoutes);
app.use('/', twitterIntegrationRoutes);
        ...
        const runFreshSet = async () => {
            ...
            for (const account of accounts) {
                ...
                if (account.bwin) {
                    ...
                    result.stories = summarize(await service.executeJob(account.uid, afterFeedJob, {
                    ...
                    if (account.reels) {
                        ...
                        result.reels = summarize(await service.executeJob(account.uid, afterStoriesJob, {
