...
export const guessInterestCategory = (message) => {
    const text = normalize(message);
...
export const detectResponseType = (message) => {
    const text = normalize(message);
...
export const scoreSentiment = (message) => {
    const text = normalize(message);
    let score = 0.1;
    if (text.match(/\b(thank|great|love|awesome|perfect|excited|amazing)\b/))
        score += 0.4;
    if (text.match(/\b(frustrated|angry|terrible|bad|upset|cancel|complain)\b/))
        score -= 0.6;
    if (text.match(/\b(issue|problem|delay|slow|stuck)\b/))
        score -= 0.3;
    return Math.min(1, Math.max(-1, Number(score.toFixed(2))));
};
export const extractLeadProfile = (message, fallbackName) => {
...
export const extractKeywords = (message, limit = 5) => {
    return Array.from(new Set(message
        .toLowerCase()
        .replace(/[^a-z0-9\s]/g, ' ')
        .split(/\s+/)
        .filter(token => token && !stopWords.has(token) && token.length > 3))).slice(0, limit);
};
