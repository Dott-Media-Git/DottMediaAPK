...
const normalizeHandle = (value) => value.trim().replace(/^@/, '').replace(/\?.*$/, '').replace(/\/$/, '');
const extractRecipient = (raw) => {
    const trimmed = raw.trim();
    if (!trimmed)
        return null;
    if (/^\d+$/.test(trimmed))
        return { userId: trimmed, handle: null };
    const byUrl = trimmed.match(/(?:x|twitter)\.com\/([a-z0-9_]{1,15})/i);
    if (byUrl?.[1])
        return { userId: null, handle: normalizeHandle(byUrl[1]) };
    const byHandle = trimmed.match(/^@?([a-z0-9_]{1,15})$/i);
    if (byHandle?.[1])
        return { userId: null, handle: normalizeHandle(byHandle[1]) };
    return null;
};
 ...
 */
export async function sendXDirectMessage(recipient, text, credentials) {
    if (!recipient?.trim()) {
