...
function formatThreadsError(error, fallback) {
    ...
    for (let attempt = 0; attempt < READY_ATTEMPTS; attempt += 1) {
        try {
            ...
            if (status === 'ERROR' || status === 'FAILED') {
                const detail = statusResp.data?.error_message;
...
async function publishWithRetry(accountId, accessToken, creationId) {
    let lastError;
    for (let attempt = 0; attempt < PUBLISH_RETRIES; attempt += 1) {
        try {
            const publishResp = await axios.post(`${GRAPH_BASE_URL}/${GRAPH_VERSION}/${accountId}/threads_publish`, null, {
                params: {
