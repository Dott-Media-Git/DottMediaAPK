...
const buildTitle = (caption, isVideo) => {
    const trimmed = caption.trim();
    if (!trimmed)
        return isVideo ? 'Video update' : 'Image update';
    if (trimmed.length <= 80)
        return trimmed;
    return `${trimmed.slice(0, 77).trim()}...`;
};
const ensureText = (caption) => (caption?.trim()?.length ? caption.trim() : ' ');
const getLinkedInHeaders = (accessToken) => ({
    Authorization: `Bearer ${accessToken}`,
    'X-Restli-Protocol-Version': '2.0.0',
});
const fetchPersonUrn = async (accessToken) => {
...
const resolveAuthorUrn = async (raw, accessToken) => {
    const trimmed = raw?.trim() ?? '';
    if (trimmed && trimmed.startsWith('urn:li:'))
        return trimmed;
    if (trimmed && /^\d+$/.test(trimmed))
        return `urn:li:organization:${trimmed}`;
    if (ALLOW_PERSON_FALLBACK)
        return await fetchPersonUrn(accessToken);
    return '';
};
const registerUpload = async (accessToken, owner, recipe) => {
    const response = await axios.post(`${LINKEDIN_API}/assets?action=registerUpload`, {
...
const uploadMedia = async (uploadUrl, mediaUrl, contentType) => {
...
const createUgcPost = async (accessToken, author, caption, mediaCategory, asset) => {
    const payload = {
        author,
...
export async function publishToLinkedIn(input) {
    ...
    const fallbackAuthor = process.env.LINKEDIN_AUTHOR_URN?.trim() || process.env.LINKEDIN_ORGANIZATION_ID?.trim() || '';
    ...
    try {
        ...
        const postId = await createUgcPost(account.accessToken, authorUrn, input.caption, isVideo ? 'VIDEO' : 'IMAGE', upload.asset);
        return { remoteId: postId ?? `li_${Date.now()}` };
    }
    catch (error) {
