...
import { classifyReply } from '../brain/nlu/replyClassifier.js';
...
export class ConversionService {
    ...
    async handleReplyWithClassification(payload, classification) {
        const result = await this.applyClassification(payload, classification);
        return { classification, ...result };
    }
    async applyClassification(payload, classification) {
        ...
        const prospect = { ...data, id: snapshot.id };
    ...
    async convertToLead(prospect, reply, classification) {
        ...
        await firestore.runTransaction(async (tx) => {
            ...
            tx.set(prospectsCollection.doc(prospect.id), {
                status: 'converted',
    ...
    async logReply(prospect, payload, classification) {
...
function resolveRecipient(prospect) {
    if (prospect.channel === 'whatsapp')
        return prospect.phone ?? undefined;
    if (prospect.channel === 'instagram')
        return prospect.profileUrl ? extractUsername(prospect.profileUrl) : undefined;
    if (prospect.channel === 'linkedin')
        return prospect.profileUrl ?? undefined;
