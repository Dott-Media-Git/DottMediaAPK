...
const MAX_PER_DAY = 5;
const DEPRECATED_CLIENT_CAMPAIGN_USER_IDS = /* @__PURE__ */ new Set([
  "acmVetCcOiTHeGk5D7eDYieamDF3",
  "D1iNgjLKNRaQhH35M0NmGfw1LVD2",
  "vzdH1DnfFLVjlY8bBgC26WACmmw2"
]);
...
class SocialPostingService {
  ...
  async getRuntimeFallbackAccounts(userId) {
    ...
    if (!this.isBwinScopeUser(userId)) {
      ...
      if (clientFallback && prefix) {
        ...
        const facebookToken2 = value("FACEBOOK_PAGE_TOKEN") || value("FACEBOOK_ACCESS_TOKEN");
        const instagramToken2 = value("INSTAGRAM_ACCESS_TOKEN") || facebookToken2;
        if (facebookToken2) {
          fallback.facebook = {
            accessToken: facebookToken2,
            pageId: value("FACEBOOK_PAGE_ID") || clientFallback.pageId
          };
        }
        if (instagramToken2) {
          fallback.instagram = {
            accessToken: instagramToken2,
    ...
    if (facebookToken && facebookPageId) {
      ...
      fallback.facebook = { accessToken: accessToken2, pageId };
    }
  ...
  toSeconds(value) {
    ...
    if (typeof value.toDate === "function") return Math.floor(value.toDate().getTime() / 1e3);
    return 0;
  }
  isLimitExempt(post) {
  ...
  async skipDeprecatedClientCampaignPost(post) {
    ...
    await this.log(post, "skipped_deprecated", void 0, message);
  }
  async runQueue(limit = 25) {
    const now = admin.firestore.Timestamp.now();
    const pendingById = /* @__PURE__ */ new Map();
    try {
      const pendingSnap = await scheduledPostsCollection.where("status", "==", "pending").where("scheduledFor", "<=", now).orderBy("scheduledFor", "asc").limit(limit).get();
      pendingSnap.docs.forEach((doc) => {
        ...
        pendingById.set(doc.id, {
          ...
          videoUrl: data.videoUrl ?? void 0,
          videoTitle: data.videoTitle ?? void 0,
          targetDate: data.targetDate ?? (/* @__PURE__ */ new Date()).toISOString().slice(0, 10),
          scheduledFor: data.scheduledFor?.toDate?.() ?? void 0,
    ...
    try {
      ...
      fallbackPosts.forEach((post) => {
        pendingById.set(post.id, {
          ...
          videoTitle: post.videoTitle ?? void 0,
          targetDate: post.targetDate ?? (/* @__PURE__ */ new Date()).toISOString().slice(0, 10),
          scheduledFor: post.scheduledFor ? new Date(post.scheduledFor) : void 0,
          source: post.source ?? void 0
        });
      });
    } catch (error) {
    ...
    for (const post of posts) {
      ...
      if (closureState?.enabled && await isBwinAccountClosureActive(post.userId)) {
        ...
        await this.log(post, "failed", void 0, message);
      ...
      try {
        ...
        if ((post.platform === "youtube" || post.platform === "tiktok" || post.platform === "instagram_reels") && !post.videoUrl) {
          await scheduledPostsCollection.doc(post.id).update({
            status: "failed",
            errorMessage: post.platform === "youtube" ? "Missing YouTube video URL" : post.platform === "tiktok" ? "Missing TikTok video URL" : "Missing Instagram Reels video URL",
            updatedAt: admin.firestore.FieldValue.serverTimestamp()
          });
          await this.log(
            post,
            "failed",
            void 0,
            post.platform === "youtube" ? "Missing YouTube video URL" : post.platform === "tiktok" ? "Missing TikTok video URL" : "Missing Instagram Reels video URL"
          );
          await socialAnalyticsService.incrementDaily({ userId: post.userId, platform: post.platform, status: "failed" });
          continue;
        }
  ...
  async getHistory(userId, limit = 400) {
    ...
    try {
      let snap;
      try {
        snap = await scheduledPostsCollection.where("userId", "==", userId).orderBy("createdAt", "desc").limit(limit).get();
      } catch (error) {
        if (!this.isMissingIndexError(error)) throw error;
        snap = await scheduledPostsCollection.where("userId", "==", userId).limit(fallbackLimit).get();
      }
    ...
    const todayPostsById = /* @__PURE__ */ new Map();
    try {
      const todaySnap = await scheduledPostsCollection.where("userId", "==", userId).where("targetDate", "==", todayDate).where("status", "==", "posted").limit(2500).get();
      ...
      const todaySeconds = Math.floor(todayStart.getTime() / 1e3);
      trimmed.forEach((post) => {
        if (post.status !== "posted") return false;
        const seconds = this.toSeconds(post.postedAt) || this.toSeconds(post.createdAt) || this.toSeconds(post.scheduledFor);
...
const socialPostingService = new SocialPostingService();
export {
