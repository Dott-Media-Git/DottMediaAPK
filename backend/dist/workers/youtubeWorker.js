...
import { getYouTubeIntegrationSecrets, updateYouTubeAccessToken } from '../services/socialIntegrationService.js';
import { cleanupTempFile, downloadVideoToTemp } from '../services/videoUrlService.js';
import { generateSoraVideoFile } from '../services/soraVideoService.js';
import { enqueueYouTubePublish, updateYouTubeJob, } from '../services/youtubeUploadService.js';
...
const buildOAuthClient = (accessToken) => {
...
const normalizeShortsMetadata = (payload) => {
    if (!payload.shorts) {
        return {
...
const uploadVideoFile = async (userId, payload, filePath) => {
...
const finalizeUploadJob = async (jobId, userId, payload, videoId, videoUrl) => {
...
if (process.env.SKIP_REDIS === 'true' || !config.redisUrl) {
    ...
    try {
        ...
        youtubeWorker = new Worker('youtube', async (job) => {
