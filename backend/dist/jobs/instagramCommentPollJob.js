...
const processComment = async (comment, target) => {
    ...
    if (!commentId || !text)
        return;
    if (fromId && fromId === target.igBusinessId)
        return;
    ...
    try {
        ...
        if (isAlreadyExistsError(error)) {
            ...
            if (legacySnap.exists && !shouldRetryExisting(legacySnap.data()))
                return;
            throw error;
        }
    }
    try {
        ...
        await likeInstagramComment(commentId, target.accessToken).catch(err => console.warn('[ig-comment-poll] like failed', err.message));
    }
    catch (error) {
...
const loadClientTargets = async () => {
    ...
    await Promise.all(userIds.map(async (userId) => {
        try {
            ...
            console.warn('[ig-comment-poll] Firestore target lookup failed', userId, error.message);
        }
        try {
            const fallback = await supabaseFallbackService.getSocialAccounts(userId);
            const instagram = fallback?.socialAccounts
                ?.instagram;
            if (instagram?.accountId && instagram.accessToken) {
                targets.push({ userId, igBusinessId: instagram.accountId, accessToken: instagram.accessToken });
            }
            ...
            console.warn('[ig-comment-poll] Supabase target lookup failed', userId, error.message);
        }
    }));
    const seen = new Set();
