...
const fetchKnownMetaHistoryPosts = async (knownProfile) => {
    ...
    if (linkedin?.accessToken && linkedin?.urn) {
        try {
            ...
            (Array.isArray(response.data?.elements) ? response.data.elements : []).forEach((post) => {
...
router.get('/social/history', requireFirebase, async (req, res, next) => {
    try {
        ...
        try {
            const userDoc = await withFallbackTimeout('social history user lookup', firestore.collection('users').doc(authUser.uid).get(), HISTORY_USER_LOOKUP_TIMEOUT_MS, null);
            if (userDoc) {
...
router.get('/social/status', requireFirebase, async (req, res, next) => {
    try {
        ...
        if (!authUser)
            return res.status(401).json({ message: 'Unauthorized' });
        const userData = await withFallbackTimeout('social status account lookup', loadStatusSocialAccounts(authUser.uid, authUser.email), SOCIAL_STATUS_LOOKUP_TIMEOUT_MS, null);
