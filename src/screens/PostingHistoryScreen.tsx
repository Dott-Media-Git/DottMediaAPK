...
const getTimestampSeconds = (timestamp?: { seconds?: number; _seconds?: number } | string | number | Date) => {
  if (!timestamp) return undefined;
  if (typeof timestamp === 'number') return timestamp > 1e12 ? Math.floor(timestamp / 1000) : Math.floor(timestamp);
...
const getPostSeconds = (post: SocialPost) =>
  getTimestampSeconds((post.postedAt ?? post.createdAt ?? post.scheduledFor) as { seconds?: number; _seconds?: number } | string | number | Date);
...
export const PostingHistoryScreen: React.FC = () => {
  ...
  const cacheKey = useMemo(
    () => `dott.postingHistory.v2:${state.user?.uid ?? 'guest'}`,
    [state.user?.uid],
  );
