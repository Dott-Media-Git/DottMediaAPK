...
export const PasswordResetScreen: React.FC<Props> = () => {
  ...
  const handleReset = async () => {
    try {
      ...
      Alert.alert('Reset failed', 'Unable to send password reset. Try again later.');
      console.error(error);
    }
  };
  ...
  return (
    ...
    >
      <View style={styles.card}>
        ...
        <Text style={styles.subtitle}>Enter the email linked to your workspace.</Text>
        <DMTextInput label="Email" value={email} onChangeText={setEmail} autoCapitalize="none" />
        <DMButton title="Send reset email" onPress={handleReset} loading={state.loading} />
      </View>
    </KeyboardAvoidingView>
  );
};
